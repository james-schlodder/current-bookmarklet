...
(function() {
  ...
  async function extractPageData() {
    ...
    // -------------------------------------------------------------------------
    // SUMMARY
    // Pull summary from first substantive paragraph in the article body
    let summary = '';
    const articleBody = document.querySelector('article p, main p, [role="main"] p');
    if (articleBody) summary = articleBody.textContent.trim();

    // Fallback to og:description if no article body paragraph found
    if (!summary) {
