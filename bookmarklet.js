...
(function() {
  ...
  async function loadPublicationMappings() {
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const publication = line.substring(commaIndex + 1).trim();
  ...
  function parseJsonLdAuthor(authorField) {
    ...
    const names = authors.map(a => {
      if (typeof a === 'string') return isUrl(a) ? '' : a.trim();
      if (typeof a === 'object' && a.name) return isUrl(a.name) ? '' : a.name.trim();
      return '';
    }).filter(Boolean);
    return names.join(', ');
  }
  ...
  function getPublicationName(jsonLd) {
    ...
    if (jsonLd && jsonLd.publisher) {
      ...
      if (typeof pub === 'string' && !isUrl(pub)) return pub;
      if (typeof pub === 'object' && pub.name) return pub.name;
    }
  ...
  async function extractPageData() {
    ...
    // -------------------------------------------------------------------------
    // SUMMARY
    // og:description is the right target — written by publishers for sharing
    // -------------------------------------------------------------------------
    let summary = '';
