...
(function() {
  ...
  async function extractPageData() {
    ...
    // -------------------------------------------------------------------------
    // SUMMARY
    // Pull first 2 sentences from the article body.
    // Skips boilerplate and the deck/subheadline (which og:description usually mirrors).
    let summary = '';
    const BOILERPLATE = /non-commercial use|subscriber agreement|copyright law|reprints|all rights reserved|terms of use|privacy policy|javascript is required/i;
    const ogDescContent = (document.querySelector('meta[property="og:description"]') || {}).content || '';

    const paragraphs = document.querySelectorAll('article p, main p, [role="main"] p');
    for (const p of paragraphs) {
      const text = p.textContent.trim();
      if (text.length < 80) continue;
      if (BOILERPLATE.test(text)) continue;
      // Skip if it matches the og:description (deck/subheadline)
      if (ogDescContent && text.includes(ogDescContent.substring(0, 60))) continue;
    ...
    // Fallback to og:description if no suitable paragraph found
    if (!summary) {
      if (ogDescContent) summary = ogDescContent.trim();
      else {
